...
Меню «Продлить подписку» работает в два шага:

1. **Шаг 1** — выбор тарифа (📦 Standard / ⭐ Premium). Кнопки показываются только если для тарифа настроен хотя бы один способ оплаты.
2. **Шаг 2** — выбор периода. Для каждого продукта отображается название и цена (если `price > 0`). Кнопка «Назад» возвращает к выбору тарифа.
...
3. Пропишите ключи в `config.yaml`:
   ...
   | `TRIBUTE_DIGITAL_PRODUCTS` | Список цифровых товаров `{name, tier, hours, price, url, referral_reward_hours}` |
